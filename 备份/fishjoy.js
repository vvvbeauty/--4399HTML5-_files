...
{	
	...
	}else
	{
		...
		Q.addMeta({name:"viewport", content:"user-scalable=no, initial-scale=1.0, minimum-scale=1, maximum-scale=1"});
		this.width =document.body.clientWidth;//Math.min(document.body.clientWidth, window.innerWidth);
		this.height = document.body.clientHeight;//Math.min(document.body.clientHeight, window.innerHeight);
	}
...
game.initUI = function()
{
	...
	this.bg.scaleX = document.body.clientWidth/320;
    this.bg.scaleY = document.body.clientHeight/480;
	...
	this.bottom.x = 0;//this.width - this.bottom.width >> 1;
	this.bottom.y = this.height - this.bottom.height + 2;
	this.bottom.transformEnabled = false;
    this.bottom.scaleX = document.body.clientWidth/765;
